...
CONFIG_VAR="no-bf no-cast no-rc2 no-rc4 no-rc5 no-dso no-dsa no-engine no-gost no-ec no-dh no-krb5 no-asm no-hw no-des no-idea no-rc2 no-zlib -DOPENSSL_NO_BUF_FREELISTS"
#Dont add these....
#no-md2 no-md5 no-mdc2 no-sha no-hmac no-ssl2 no-rsa

ARCHS="i386 x86_64 armv7 armv7s arm64"
DEVELOPER=`xcode-select -print-path`

echo "DEVELOPER Path =${DEVELOPER}"
if [ ! -d "$DEVELOPER" ]; then
    echo "xcode path is not set correctly $DEVELOPER does not exist most likely because of xcode older than 4.3"
    echo "run"
...
tar -zxvf openssl-${VERSION}.tar.gz -C "${CURRENTPATH}/src"
...
do
    ...
    if [[ "$VERSION" =~ 1.0.0. ]]; then
	    ...
	    ./Configure darwin64-x86_64-cc ${CONFIG_VAR} --openssldir="${CURRENTPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk" > "${LOG}" 2>&1
    else
